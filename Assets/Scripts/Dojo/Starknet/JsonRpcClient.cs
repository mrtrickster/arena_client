...
#if UNITY_WEBGL && !UNITY_EDITOR
        public JsonRpcClient(string rpcUrl, string chainId = "") {
            if (chainId == "")
            {
                client = StarknetInterop.NewProvider(new CString(rpcUrl));
            } else  {
                client = StarknetInterop.NewProvider(new CString(rpcUrl), new CString(chainId));
            }
        }
#else
        public unsafe JsonRpcClient(string rpcUrl, string chainId = "")
        {
            dojo.ResultProvider result;
            if (chainId == "")
            {
                result = dojo.provider_new(CString.FromString(rpcUrl));
            }
            else
            {
                result = dojo.provider_new(CString.FromString(rpcUrl), CString.FromString(chainId));
            }
